...
const PreElectionPage: React.FC = () => {
  ...
  const handleVote = async (candidateId: string) => {
    // CRITICAL: Voting confirmation must be bilingual regardless of toggle
        ...
        <div className="flex-grow space-y-4">
            {candidates.length === 0 ? (
                 <div className="text-center py-16 bg-white border border-slate-200 rounded-sm">
                    <p className="text-slate-500">{t('कुनै उम्मेदवार छैनन्।', 'No candidates registered.')}</p>
                    <Link to={`/constituency/${id}/apply`} className="mt-4 inline-block text-[#0094da] underline font-medium text-sm">Be the first candidate</Link>
                 </div>
            ) : (
                candidates.map((candidate) => (
                    <div key={candidate.id} className="bg-white rounded-sm border border-slate-200 p-6 transition hover:border-[#0094da]">
                            ...
                            <div className="flex-shrink-0 flex items-center justify-center md:justify-start">
                                <button
                                    ...
                                    className={`px-6 py-2 rounded-sm text-sm font-bold uppercase tracking-wide transition border ${
                                        ...
                                        : 'bg-[#0094da] text-white border-transparent hover:bg-[#007bb8] shadow-sm'
                                    }`}
                                >
        ...
        <div className="w-full lg:w-72 flex-shrink-0">
             <div className="bg-white rounded-sm border border-slate-200 p-5 sticky top-24">
                    ...
                    {candidates.slice(0, 3).map((c, index) => (
                        <div key={c.id} className="flex items-center justify-between py-3">
                            <div className="flex items-center">
                                <span className={`w-5 h-5 rounded-sm flex items-center justify-center text-[10px] font-bold mr-3 ${index === 0 ? 'bg-[#0094da] text-white' : 'bg-slate-100 text-slate-600'}`}>
                                    {index + 1}
                                </span>
                                <div>
                 ...
                 <div className="mt-6 pt-4 border-t border-slate-100">
                     <Link to={`/constituency/${id}/apply`} className="block w-full text-center bg-white border border-slate-300 text-slate-700 py-2 rounded-sm text-xs font-bold uppercase hover:bg-slate-50 hover:text-[#0094da]">
